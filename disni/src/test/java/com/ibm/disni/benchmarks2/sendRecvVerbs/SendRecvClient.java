 ...
 *  java -cp disni-1.6-jar-with-dependencies.jar:disni-1.6-tests.jar com.ibm.disni.benchmarks2.sendRecvVerbs.SendRecvClient -a 10.10.0.93 -k 1000 -s 16
 */
public class SendRecvClient implements RdmaEndpointFactory<SendRecvClient.CustomClientEndpoint> {
	...
	public SendRecvClient.CustomClientEndpoint createEndpoint(RdmaCmId idPriv, boolean serverSide) throws IOException {
		return new CustomClientEndpoint(endpointGroup, idPriv, serverSide, size);
	}
...
//        postSend.getWrMod(0).setWr_id(4444);
        for (int i = 0; i < loop; i++) {
            ...
            SVCPostSend postSend = endpoint.postSend(wrList_send);
...
//            System.out.println("SimpleClient, wcEvents length: " + endpoint.getWcEvents().size());
//            System.out.println("SimpleClient::message sent, wr_id " + wc.getWr_id());
        }
        long endTime = System.nanoTime();
        long duration = endTime - startTime;
        double latency = duration / 1e6 / loop;
        System.out.println("SimpleClient, duration: " + duration / 1e6 + "ms, loop: " + loop);
        double throughput = loop / (duration / 1e9);
        System.out.println("SimpleClient, throughput: " + throughput + ", latency: " + latency);
	...
	public void launch(String[] args) throws Exception {
        RdmaBenchmarkCmdLine cmdLine = new RdmaBenchmarkCmdLine("SendRecvClient");
