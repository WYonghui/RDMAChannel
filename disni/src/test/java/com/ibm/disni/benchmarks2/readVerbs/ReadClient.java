 ...
 * java -cp disni-1.6-jar-with-dependencies.jar:disni-1.6-tests.jar com.ibm.disni.benchmarks2.readVerbs.ReadClient -a 10.10.0.93 -s 16 -k 1
 */
public class ReadClient implements RdmaEndpointFactory<ReadClient.ReadClientEndpoint> {
    private RdmaPassiveEndpointGroup<ReadClientEndpoint> endpointGroup;
    private String host;
    private int port;
    private int size;
    private int loop;
    ...
    public ReadClient(String host, int port, int size, int loop) throws IOException{
        this.endpointGroup = new RdmaPassiveEndpointGroup<ReadClient.ReadClientEndpoint>(1, 10, 4, 40);
    ...
    @Override
    public ReadClientEndpoint createEndpoint(RdmaCmId id, boolean serverSide) throws IOException {
        return new ReadClient.ReadClientEndpoint(endpointGroup, id, serverSide ,size);
    }
    ...
    public void run() throws Exception {
        ...
        ReadClient.ReadClientEndpoint endpoint = endpointGroup.createEndpoint();
        ...
        for (int i = 0; i < loop; i++){
            ...
            dataBuf.clear();
//            System.out.println("RdmaPassiveReadClient::read memory from server: ");
...
//            System.out.println();
    ...
    public static class ReadClientEndpoint extends RdmaEndpoint {
        ...
        public ReadClientEndpoint(RdmaEndpointGroup<? extends RdmaEndpoint> endpointGroup, RdmaCmId idPriv, boolean isServerSide, int size) throws IOException {
